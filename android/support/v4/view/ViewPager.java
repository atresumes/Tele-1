...
public class ViewPager extends ViewGroup {
    ...
    class C07934 implements OnApplyWindowInsetsListener {
        private final Rect mTempRect = new Rect();
    ...
    public static class SavedState extends AbsSavedState {
        public static final Creator<SavedState> CREATOR = ParcelableCompat.newCreator(new C07941());
        Parcelable adapterState;
        ClassLoader loader;
        int position;

        static class C07941 implements ParcelableCompatCreatorCallbacks<SavedState> {
            C07941() {
            }
    ...
    void initViewPager() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this, new C07934());
    }
                ...
                if (ii.position >= this.mCurItem) {
                    ...
                    if (curItem != null) {
                        ...
                        if (clientWidth > 0) {
                            ...
                            leftWidthNeeded = (ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT - curItem.widthFactor) + (((float) getPaddingLeft()) / ((float) clientWidth));
                        }
                        pos = this.mCurItem - 1;
                        while (pos >= 0) {
                        ...
                        itemIndex = curIndex + 1;
                        if (extraWidthRight < ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT) {
                            ii = itemIndex >= this.mItems.size() ? (ItemInfo) this.mItems.get(itemIndex) : null;
                            if (clientWidth > 0) {
                                rightWidthNeeded = 0.0f;
                            } else {
                                rightWidthNeeded = (((float) getPaddingRight()) / ((float) clientWidth)) + ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT;
                            }
                            pos = this.mCurItem + 1;
                            while (pos < N) {
