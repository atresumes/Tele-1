...
import com.payu.magicretry.C0537R;
...
public class DotsTextView extends TextView {
    ...
    class C05381 implements AnimatorUpdateListener {
        C05381() {
        }
    ...
    class C05392 implements TypeEvaluator<Number> {
        C05392() {
        }
    ...
    class C05403 implements AnimatorUpdateListener {
        C05403() {
        }
    ...
    class C05414 implements AnimatorUpdateListener {
        C05414() {
        }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray typedArray = context.obtainStyledAttributes(attrs, C0537R.styleable.WaitingDots);
            this.period = typedArray.getInt(C0537R.styleable.WaitingDots_period, 6000);
            this.jumpHeight = typedArray.getInt(C0537R.styleable.WaitingDots_jumpHeight, (int) (getTextSize() / 4.0f));
            this.autoPlay = typedArray.getBoolean(C0537R.styleable.WaitingDots_autoplay, true);
            typedArray.recycle();
        }
        ...
        spannable.setSpan(this.dotOne, 0, 1, 33);
        spannable.setSpan(this.dotTwo, 1, 2, 33);
        spannable.setSpan(this.dotThree, 2, 3, 33);
        setText(spannable, BufferType.SPANNABLE);
        this.textWidth = getPaint().measureText(".", 0, 1);
        createDotJumpAnimator(this.dotOne, 0).addUpdateListener(new C05381());
    ...
    private ObjectAnimator createDotJumpAnimator(JumpingSpan jumpingSpan, long delay) {
        ...
        jumpAnimator.setEvaluator(new C05392());
    ...
    public void hide() {
        createDotHideAnimator(this.dotThree, ImageRequest.DEFAULT_IMAGE_BACKOFF_MULT).start();
        ObjectAnimator dotTwoMoveRightToLeft = createDotHideAnimator(this.dotTwo, DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        dotTwoMoveRightToLeft.addUpdateListener(new C05403());
    ...
    public void show() {
        ...
        dotTwoMoveRightToLeft.addUpdateListener(new C05414());
        dotTwoMoveRightToLeft.start();
        this.isHide = false;
    }
