package com.paypal.android.sdk;

import android.content.Context;
import android.view.View;
import android.view.ViewGroup;
import android.view.ViewGroup.LayoutParams;
import android.widget.ArrayAdapter;
import android.widget.LinearLayout;
import android.widget.RelativeLayout;
import android.widget.TextView;
import java.util.List;

public final class gh extends ArrayAdapter {
    private int f555a;

    public gh(Context context, List list, int i) {
        super(context, 0, list);
        this.f555a = i;
    }

    public final void m419a(int i) {
        this.f555a = i;
    }

    public final View getView(int i, View view, ViewGroup viewGroup) {
        String str = (String) getItem(i);
        View linearLayout = new LinearLayout(viewGroup.getContext());
        View relativeLayout = new RelativeLayout(viewGroup.getContext());
        linearLayout.addView(relativeLayout);
        cz.m240a(relativeLayout, null, "6dip", null, "6dip");
        View a = cz.m229a(viewGroup.getContext(), "", "checked");
        a.setId(2303);
        LayoutParams a2 = cz.m233a(viewGroup.getContext(), "20dip", "20dip", 11);
        a2.addRule(10);
        relativeLayout.addView(a, a2);
        a.setColorFilter(cy.f247b);
        cz.m240a(a, null, null, "8dip", null);
        if (i != this.f555a) {
            a.setVisibility(4);
        }
        TextView textView = new TextView(viewGroup.getContext());
        textView.setId(2302);
        cz.m250b(textView, 83);
        a2 = cz.m233a(viewGroup.getContext(), "20dip", "20dip", 9);
        a2.addRule(0, 2303);
        relativeLayout.addView(textView, a2);
        cz.m248b(textView, "6dip", "6dip", "6dip", "6dip");
        textView.setText(str);
        return linearLayout;
    }
}
