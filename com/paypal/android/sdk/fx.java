package com.paypal.android.sdk;

import android.content.Context;
import android.text.method.LinkMovementMethod;
import android.view.View;
import android.view.ViewGroup;
import android.view.ViewGroup.LayoutParams;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.TextView;
import com.android.volley.DefaultRetryPolicy;

public final class fx {
    public ViewGroup f474a;
    public TextView f475b;
    public TextView[] f476c = new TextView[10];
    public TextView f477d;
    public TextView f478e;
    public fy f479f;
    public Button f480g;
    public LinearLayout f481h;
    public TextView f482i;
    private ViewGroup f483j;
    private LinearLayout f484k;
    private ImageView f485l;

    public fx(Context context) {
        this.f483j = cz.m228a(context);
        this.f484k = cz.m231a(this.f483j);
        LinearLayout b = cz.m246b(this.f484k);
        this.f475b = new TextView(context);
        cz.m240a(this.f475b, "0dip", "0dip", "0dip", "14dip");
        this.f475b.setTextSize(24.0f);
        this.f475b.setTextColor(cy.f246a);
        b.addView(this.f475b);
        cz.m236a(this.f475b, -2, -2);
        this.f485l = new ImageView(context);
        this.f485l.setImageBitmap(cz.m251c("", context));
        this.f485l.setContentDescription("PayPal");
        b.addView(this.f485l);
        cz.m237a(this.f485l, -1, "40dip");
        cz.m248b(this.f485l, null, null, null, "10dip");
        for (int i = 0; i < 10; i++) {
            TextView[] textViewArr = this.f476c;
            View textView = new TextView(context);
            textView.setTextColor(cy.f256k);
            textView.setLinkTextColor(cy.f265t);
            textView.setTypeface(cy.f264s);
            textView.setTextSize(18.0f);
            textView.setSingleLine(false);
            textView.setMovementMethod(LinkMovementMethod.getInstance());
            b.addView(textView);
            cz.m236a(textView, -1, -2);
            cz.m239a(textView, null, "10dip");
            textViewArr[i] = textView;
            this.f476c[i].setFocusable(false);
            this.f476c[i].setVisibility(8);
            this.f476c[i].setId(i + 100);
        }
        this.f477d = new TextView(context);
        this.f477d.setId(2);
        cz.m249b(this.f477d);
        b.addView(this.f477d);
        cz.m236a(this.f477d, -1, -2);
        cz.m239a(this.f477d, "20dip", "10dip");
        this.f477d.setNextFocusRightId(1);
        this.f478e = new TextView(context);
        this.f478e.setId(4);
        cz.m249b(this.f478e);
        b.addView(this.f478e);
        cz.m236a(this.f478e, -1, -2);
        cz.m248b(this.f478e, null, "20dip", null, "10dip");
        this.f478e.setVisibility(8);
        this.f481h = cz.m230a(context, true, 1, b);
        this.f481h.setNextFocusLeftId(2);
        this.f481h.setNextFocusRightId(3);
        this.f482i = new TextView(context);
        cz.m243a(this.f482i);
        this.f482i.setText("Agree");
        this.f481h.addView(this.f482i, new LayoutParams(-2, -2));
        this.f480g = new Button(context);
        this.f480g.setId(3);
        cz.m242a(this.f480g);
        b.addView(this.f480g);
        cz.m248b(this.f480g, null, null, null, "16dip");
        cz.m236a(this.f480g, -2, -2);
        cz.m235a(this.f480g, 17, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        this.f480g.setNextFocusLeftId(1);
        this.f479f = new fy(context);
        this.f484k.addView(this.f479f.f486a);
        cz.m236a(this.f479f.f486a, -2, -2);
        cz.m235a(this.f479f.f486a, 17, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        this.f474a = this.f483j;
    }
}
