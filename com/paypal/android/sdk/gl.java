package com.paypal.android.sdk;

import android.util.Log;
import com.talktoangel.gts.utils.SessionManager;
import java.util.ArrayList;
import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

public final class gl implements go {
    private String f1012a;
    private String f1013b;
    private String f1014c;
    private String f1015d;
    private String f1016e;
    private String f1017f;
    private String f1018g;
    private String f1019h;
    private String f1020i;

    static {
        gl.class.getSimpleName();
    }

    public gl() {
        this("", null, null, null, null, null, null, null);
    }

    private gl(String str, String str2, String str3, String str4, String str5, String str6, String str7, String str8) {
        this.f1012a = fu.m369a(fw.SHIPPING_ADDRESS);
        this.f1013b = str;
        this.f1014c = str2;
        this.f1015d = str3;
        this.f1016e = str4;
        this.f1017f = str5;
        this.f1018g = str6;
        this.f1019h = str7;
        this.f1020i = str8;
    }

    public static ArrayList m869a(JSONObject jSONObject, JSONArray jSONArray, int i) {
        ArrayList arrayList = new ArrayList();
        if (jSONObject != null) {
            try {
                m870a(arrayList, jSONObject);
            } catch (JSONException e) {
                Log.e("paypal.sdk", e.getMessage());
            }
        }
        if (jSONArray != null) {
            for (int i2 = 0; i2 < jSONArray.length(); i2++) {
                if (i2 != i) {
                    m870a(arrayList, jSONArray.getJSONObject(i2));
                }
            }
        }
        return arrayList;
    }

    private static void m870a(ArrayList arrayList, JSONObject jSONObject) {
        if (jSONObject != null) {
            jSONObject.optString("type");
            arrayList.add(new gl("", jSONObject.getString("recipient_name"), jSONObject.getString("line1"), jSONObject.optString("line2"), jSONObject.getString("city"), jSONObject.getString("state"), jSONObject.getString("postal_code"), jSONObject.getString(SessionManager.COUNTRY_CODE)));
        }
    }

    public final String mo2174a() {
        return this.f1013b;
    }

    public final String mo2175b() {
        return this.f1012a;
    }

    public final String mo2176c() {
        return this.f1015d;
    }

    public final String mo2177d() {
        return this.f1019h;
    }

    public final boolean mo2178e() {
        return false;
    }

    public final String m876f() {
        return this.f1014c;
    }

    public final String m877g() {
        return this.f1015d;
    }

    public final String m878h() {
        return this.f1016e;
    }

    public final String m879i() {
        return this.f1017f;
    }

    public final String m880j() {
        return this.f1018g;
    }

    public final String m881k() {
        return this.f1019h;
    }

    public final String m882l() {
        return this.f1020i;
    }
}
