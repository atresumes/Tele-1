package com.paypal.android.sdk.payments;

import android.content.Context;
import android.view.View;
import android.webkit.WebView;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.ScrollView;
import android.widget.TextView;
import com.paypal.android.sdk.C0441d;
import com.paypal.android.sdk.cy;
import com.paypal.android.sdk.cz;
import com.paypal.android.sdk.fu;
import com.paypal.android.sdk.fw;
import java.util.Arrays;
import java.util.HashSet;
import java.util.Locale;

final class ah {
    View f763a;
    TextView f764b;
    Button f765c;
    private ScrollView f766d;
    private LinearLayout f767e;
    private ImageView f768f;

    ah(Context context, ag agVar) {
        String format;
        this.f766d = new ScrollView(context);
        this.f766d.setBackgroundColor(cy.f249d);
        this.f767e = new LinearLayout(context);
        this.f767e.setOrientation(1);
        this.f766d.addView(this.f767e);
        cz.m236a(this.f767e, -1, -2);
        View linearLayout = new LinearLayout(context);
        linearLayout.setOrientation(1);
        cz.m240a(linearLayout, "16dip", "20dip", "16dip", "20dip");
        this.f767e.addView(linearLayout);
        cz.m236a(linearLayout, -1, -2);
        this.f768f = new ImageView(context);
        this.f768f.setImageBitmap(cz.m251c("", context));
        this.f768f.setContentDescription("PayPal");
        linearLayout.addView(this.f768f);
        cz.m237a(this.f768f, -1, "40dip");
        cz.m248b(this.f768f, null, null, null, "10dip");
        this.f764b = new TextView(context);
        View webView = new WebView(context);
        webView.setBackgroundColor(cy.f249d);
        StringBuilder append = new StringBuilder("<html><meta name='viewport' content='width=device-width; initial-scale=1.0;'/><head><style type=\"text/css\">").append("body { color: #515151; line-height: 26px; font-size: 18px; font-family: sans-serif-light, DroidSans, sans-serif; font-weight: 300; " + (fu.f419a ? "direction: rtl;" : "") + "} h1, strong { font-family: DroidSans, sans-serif; font-weight: 500; }" + "h1 { font-size: 18px }h1, p { margin: 0 0 13px 0 }" + "ul { margin: 0; padding-left: 1.1em; list-style-position: outside; }").append("</style></head><body>");
        switch (ai.f769a[agVar.ordinal()]) {
            case 1:
                String a = fu.m369a(fw.FUTURE_PAYMENT_LEGAL_DETAILS);
                Object toLowerCase = Locale.getDefault().getCountry().toLowerCase(Locale.US);
                if (C0441d.m267c((CharSequence) toLowerCase)) {
                    toLowerCase = "us";
                }
                format = new HashSet(Arrays.asList(new String[]{"nl", "de", "it", "fr", "gb", "us"})).contains(toLowerCase) ? String.format("https://www.paypal.com/%s/webapps/mpp/ua/useragreement-full", new Object[]{toLowerCase}) : "";
                format = String.format(a, new Object[]{format});
                break;
            case 2:
                format = fu.m369a(fw.FINANCIAL_INSTRUMENTS_LEGAL_DETAILS);
                break;
            case 3:
                format = String.format(fu.m369a(fw.REQUEST_OR_SEND_MONEY_LEGAL_DETAILS), new Object[]{fu.m369a(fw.REQUEST_MONEY)});
                break;
            case 4:
                format = String.format(fu.m369a(fw.REQUEST_OR_SEND_MONEY_LEGAL_DETAILS), new Object[]{fu.m369a(fw.SEND_MONEY)});
                break;
            default:
                format = null;
                break;
        }
        webView.loadData(append.append(format).append("</body><html>").toString(), "text/html; charset=UTF-8", null);
        webView.setClickable(true);
        linearLayout.addView(webView);
        cz.m236a(webView, -1, -2);
        cz.m248b(webView, null, null, null, "10dip");
        this.f765c = new Button(context);
        linearLayout.addView(this.f765c);
        cz.m241a(this.f765c, true, context);
        cz.m248b(this.f765c, null, "20dip", null, "4dip");
        this.f763a = this.f766d;
    }
}
