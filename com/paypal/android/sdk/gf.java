package com.paypal.android.sdk;

import android.content.Context;
import android.util.TypedValue;
import android.view.View;
import android.view.ViewGroup;
import android.view.ViewGroup.LayoutParams;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.ImageView.ScaleType;
import android.widget.LinearLayout;
import android.widget.TextView;
import com.android.volley.DefaultRetryPolicy;

public final class gf {
    public ViewGroup f537a;
    public Button f538b;
    public gk f539c;
    public TextView f540d;
    public TextView f541e;
    public TextView f542f;
    public ImageView f543g;
    public LinearLayout f544h;
    public fy f545i;
    public LinearLayout f546j;
    public Button f547k;
    public TextView f548l;
    public ImageView f549m;
    private LinearLayout f550n = cz.m231a(this.f537a);

    public gf(Context context) {
        this.f537a = cz.m228a(context);
        LinearLayout b = cz.m246b(this.f550n);
        this.f548l = new TextView(context);
        this.f548l.setText("Your Order");
        cz.m240a(this.f548l, "0dip", "0dip", "0dip", "14dip");
        this.f548l.setTextSize(24.0f);
        this.f548l.setTextColor(cy.f246a);
        b.addView(this.f548l, new LayoutParams(-2, -2));
        this.f539c = new gk(context, "$0.00");
        b.addView(this.f539c.f568b);
        cz.m239a(this.f539c.f568b, null, "16dip");
        this.f546j = cz.m230a(context, true, 42044, b);
        View imageView = new ImageView(context);
        imageView.setImageBitmap(cz.m251c("", context));
        imageView.setScaleType(ScaleType.CENTER_INSIDE);
        imageView.setAdjustViewBounds(true);
        imageView.setContentDescription("PayPal");
        if (fu.f419a) {
            this.f546j.addView(imageView);
            cz.m248b(imageView, "4dip", "2.5dip", null, null);
            cz.m237a(imageView, -2, "28dip");
        }
        this.f542f = new TextView(context);
        cz.m243a(this.f542f);
        this.f542f.setText("Pay with");
        this.f546j.addView(this.f542f);
        if (!fu.f419a) {
            this.f546j.addView(imageView);
            cz.m248b(imageView, "4dip", "2.5dip", null, null);
            cz.m237a(imageView, -2, "28dip");
        }
        imageView = new LinearLayout(context);
        imageView.setOrientation(1);
        b.addView(imageView, new LinearLayout.LayoutParams(-1, -2));
        cz.m248b(imageView, null, null, null, "16dip");
        this.f540d = new TextView(context);
        this.f540d.setTextSize(14.0f);
        this.f540d.setTextColor(cy.f257l);
        this.f540d.setGravity(17);
        imageView.addView(this.f540d, new LinearLayout.LayoutParams(-1, -2));
        cz.m248b(this.f540d, null, "8dip", null, "4dip");
        this.f538b = new Button(context);
        imageView.addView(this.f538b);
        cz.m242a(this.f538b);
        cz.m237a(this.f538b, -2, "24dip");
        cz.m235a(this.f538b, 17, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        this.f544h = cz.m230a(context, false, 42043, b);
        this.f549m = new ImageView(context);
        this.f549m.setAdjustViewBounds(true);
        this.f549m.setContentDescription("PayPal");
        this.f549m.setScaleType(ScaleType.CENTER_INSIDE);
        this.f549m.setImageBitmap(cz.m251c("", context));
        this.f544h.addView(this.f549m, new LayoutParams(-2, (int) TypedValue.applyDimension(1, 22.0f, context.getResources().getDisplayMetrics())));
        cz.m248b(this.f549m, null, null, "6dip", "1dip");
        cz.m237a(this.f549m, -2, "22dip");
        this.f549m.setVisibility(8);
        this.f543g = new ImageView(context);
        this.f543g.setAdjustViewBounds(true);
        this.f543g.setContentDescription("PayPal");
        this.f543g.setScaleType(ScaleType.CENTER_INSIDE);
        this.f543g.setVisibility(8);
        this.f544h.addView(this.f543g, new LayoutParams(-2, (int) TypedValue.applyDimension(1, 22.0f, context.getResources().getDisplayMetrics())));
        cz.m248b(this.f543g, null, null, "4dip", null);
        cz.m247b(this.f543g, "36dip", "25dip");
        this.f541e = new TextView(context);
        cz.m243a(this.f541e);
        this.f541e.setText("Pay with Card");
        this.f544h.addView(this.f541e, new LayoutParams(-2, -2));
        this.f547k = new Button(context);
        this.f547k.setId(42042);
        cz.m242a(this.f547k);
        b.addView(this.f547k);
        cz.m248b(this.f547k, null, null, null, "16dip");
        cz.m236a(this.f547k, -2, -2);
        cz.m235a(this.f547k, 17, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
        this.f545i = new fy(context);
        b.addView(this.f545i.f486a);
        cz.m236a(this.f545i.f486a, -2, -2);
        cz.m235a(this.f545i.f486a, 17, (float) DefaultRetryPolicy.DEFAULT_BACKOFF_MULT);
    }
}
