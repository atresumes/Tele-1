...
public class PayUmoneySdkInitilizer {
    ...
    private static Boolean isDebugMode = Boolean.valueOf(false);
    ...
    public static class PaymentParam {
        ...
        private PaymentParam(Builder builder) {
            ...
            if (builder.amount >= 0.0d || builder.amount <= 1000000.0d) {
                ...
                String hash = hashCal(this.pipedHash);
                if (PayUmoneySdkInitilizer.IsDebugMode().booleanValue()) {
                    Log.d(SdkConstants.HASH, hash);
                }
                setMerchantHash(hash);
                if (PayUmoneySdkInitilizer.IsDebugMode().booleanValue()) {
                    for (String key : this.params.keySet()) {
                        Log.d("param : ", key + " - " + ((String) this.params.get(key)));
                    }
                    return;
                }
                return;
        ...
        public void setMerchantHash(String serverCalculatedHash) {
            this.params.put(SdkConstants.HASH, serverCalculatedHash);
        }
    ...
    public static void setDebugMode(boolean isDebugMode) {
        isDebugMode = Boolean.valueOf(isDebugMode);
        setBaseUrl(isDebugMode ? BASE_URL_DEBUG : BASE_URL_LIVE);
        setBaseUrlImage(isDebugMode ? BASE_URL_IMAGE_DEBUG : BASE_URL_IMAGE_LIVE);
        setWebviewRedirectionUrl(isDebugMode ? WEBVIEW_REDIRECTION_URL_DEBUG : "https://secure.payu.in/_seamless_payment");
    }
